...
function toolchain(_buildDir, _libDir)
	print("configuring toolchain for:")
	print("\t_buildDir", _buildDir)
	print("\t_libDir", _libDir)
...
--			premake.gcc.llvm = true
	...
	elseif _ACTION == "vs2012"
		...
		if (_ACTION .. "-clang") == _OPTIONS["vs"] then
			if "vs2017-clang" == _OPTIONS["vs"] then
				premake.vstudio.toolset = "LLVM-vs2014" --"v141_clang_c2"
	...
	flags {
		"ExtraWarnings", -- scaffolding
		"StaticRuntime", -- scaffolding
		"Unicode", -- scaffolding
		"NoPCH",
		"NativeWChar",
		--"NoRTTI", -- scaffolding
		--"NoExceptions", -- scaffolding
		"NoEditAndContinue",
		"NoFramePointer",
		"Symbols",
	}
...
--			"-Wuseless-cast",
	...
	configuration { "osx", "Universal32" }
		targetdir (path.join(_buildDir, "osx_universal32/bin"))
		objdir (path.join(_buildDir, "osx_universal32/bin"))

	configuration { "osx", "Universal64" }
		targetdir (path.join(_buildDir, "osx_universal64/bin"))
		objdir (path.join(_buildDir, "osx_universal64/bin"))
	...
	configuration { "osx" }
		...
		defines {
			"GL_SILENCE_DEPRECATION", --OpenGL sanity
		}
